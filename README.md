...
| **结巴** | 是/否 | - | 每轮30%概率（情绪波动时40%），每段对话只触发一次 |
...
v1.4.0 — 新增配置项 `persona_style_extra`（额外风格说明）、`save_conversation_log`（是否持久化对话日志到磁盘）、`stutter_probability`（结巴概率）；修复 `unrestricted_list` 列表/字符串兼容；结巴由插件接管不再依赖 reply_rules
